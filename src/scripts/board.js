...
function canReportContent(content) {
  if (!currentUser || !content) return false;
  return content.profiles?.id !== currentUser.id;
}
...
function renderThreadControls(thread) {
  ...
  if (canReportContent(thread)) {
    controls.push(`<button class="btn report-content-button" id="reportThread" type="button">Report</button>`);
  }
...
function renderReplyControls(post) {
  ...
  if (canReportContent(post)) {
    controls.push(`<button class="btn report-content-button report-reply-button" type="button" data-post-id="${escapeHtml(post.id)}">Report</button>`);
  }
...
function openReportDialog(target) {
  if (!reportModal || !currentUser || !target?.id || !target?.type) return;

  pendingReportTarget = target;
  reportReason.value = "";
  reportDetails.value = "";
  reportStatus.textContent = "Reports are private and visible only to you and the moderation team.";
  reportTargetSummary.textContent = target.label || "Tell the moderation team what needs attention.";
...
async function submitContentReport(event) {
  ...
  if (!currentUser || !pendingReportTarget) {
    reportStatus.textContent = "Sign in before submitting a report.";
    return;
  }

  if (!reportReason.value) {
    reportStatus.textContent = "Choose a report reason.";
    reportReason.focus();
    return;
  }
  ...
  const payload = {
    ...
    thread_id: pendingReportTarget.type === "thread" ? pendingReportTarget.id : null,
    post_id: pendingReportTarget.type === "post" ? pendingReportTarget.id : null
  };
  const { error } = await supabase.from("board_reports").insert(payload);

  if (error) {
    console.error("Report submission failed:", error);
    reportStatus.textContent = error.message || "The report could not be submitted.";
    submitReport.disabled = false;
    return;
  }
...
function attachReplyControlListeners() {
  ...
  document.querySelectorAll(".report-reply-button").forEach((button) => {
    button.addEventListener("click", (event) => {
      event.preventDefault();
      event.stopPropagation();
      const post = loadedPosts.find((item) => item.id === button.dataset.postId);
      if (!post) return;
      openReportDialog({
        type: "post",
        id: post.id,
        label: `Report reply by ${getProfileName(post.profiles || {})}`
      });
    });
  });
...
function attachAdminControlListeners(thread) {
  ...
  const reportThreadButton = document.getElementById("reportThread");

  if (reportThreadButton) {
    reportThreadButton.addEventListener("click", (event) => {
      event.preventDefault();
      openReportDialog({
        type: "thread",
        id: thread.id,
        label: `Report thread by ${getProfileName(thread.profiles || {})}`
      });
    });
  }
...
if (reportForm) {
  reportForm.addEventListener("submit", submitContentReport);
}

if (closeReportModal) {
  closeReportModal.addEventListener("click", hideReportDialog);
}
