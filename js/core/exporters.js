...
export function exportJson() {
  // Use pixels by default unless a "scale out" checkbox exists and is unchecked
  ...
  for (let i = 0; i < N; i++) {
    const p = kps[i];
    if (!p || p.x == null || p.y == null || p.missing) {
      body.push(0, 0, 0);
    } else {
      const x = usePixels ? p.x : p.x / canvas.width;
      const y = usePixels ? p.y : p.y / canvas.height;
      body.push(x, y, p.c ?? 1);
    }
  }
  ...
  for (let i = 0; i < 21; i++) {
    ...
    if (!lp || lp.x == null || lp.y == null || lp.missing) {
      ...
      const x = usePixels ? lp.x : lp.x / canvas.width;
      const y = usePixels ? lp.y : lp.y / canvas.height;
      L.push(x, y, lp.c ?? 1);
    }
    ...
    if (!rp || rp.x == null || rp.y == null || rp.missing) {
      ...
      const x = usePixels ? rp.x : rp.x / canvas.width;
      const y = usePixels ? rp.y : rp.y / canvas.height;
      R.push(x, y, rp.c ?? 1);
    }
  }
  ...
  downloadBlob(new Blob([JSON.stringify(json, null, 2)], { type: 'application/json' }),
               'openpose_body25_hands.json');
}
...
export function exportPosePng() {
  ...
  function handSegs(handArr, limbKey) {
    ...
    // Bridge wrist to body wrist (OpenPose indices: 4=RWrist, 7=LWrist)
